...
class Return(Base):
    ...
    item_condition = Column(SQLEnum(ItemCondition, values_callable=lambda x: [e.value for e in x]), nullable=True)
    status = Column(SQLEnum(ReturnStatus, values_callable=lambda x: [e.value for e in x]), default=ReturnStatus.REQUESTED, nullable=False, index=True)
